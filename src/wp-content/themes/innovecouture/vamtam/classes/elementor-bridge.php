...
class VamtamElementorBridge {
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'innovecouture' ),
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'innovecouture' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'innovecouture' ),
				'h1'           => __( 'H1', 'innovecouture' ),
				'h2'           => __( 'H2', 'innovecouture' ),
				'h3'           => __( 'H3', 'innovecouture' ),
				'h4'           => __( 'H4', 'innovecouture' ),
				'h5'           => __( 'H5', 'innovecouture' ),
				'h6'           => __( 'H6', 'innovecouture' ),
			];
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'innovecouture' ), $font_title ),
	...
	protected static function options_exist( $option ) {
		...
		} elseif ( $option === 'theme-custom-colors' ) {
			if ( is_array( $el_global_defs ) ) {
				// Use elementor-global-defaults.php values.
				return $el_global_defs['custom_colors'];
			} else {
				$options_exist = false;
			}
		} elseif ( $option === 'theme-custom-fonts' ) {
			if ( is_array( $el_global_defs ) ) {
				// Use elementor-global-defaults.php values.
				return $el_global_defs['custom_typography'];
			} else {
				$options_exist = false;
			}
		} else {
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'innovecouture' ),
				'multiple' => true,
	...
	public static function get_widget_mods_list() {
		...
		*/
		...
		$widget_mods_list = [
			...
			'button'                                     => [ 'label' => __( 'Button', 'innovecouture' ) ],
			'call-to-action'                             => [ 'label' => __( 'Call To Action', 'innovecouture' ) ],
			'form'                                       => [ 'label' => __( 'Form', 'innovecouture' ) ],
			'loop-carousel'                              => [ 'label' => __( 'Loop Carousel', 'innovecouture' ) ],
			'loop-grid'                              	 => [ 'label' => __( 'Loop Grid', 'innovecouture' ) ],
			'login'                                      => [ 'label' => __( 'Login', 'innovecouture' ) ],
			'tabs'                                       => [ 'label' => __( 'Tabs', 'innovecouture' ) ],
			'nav-menu'                                   => [ 'label' => __( 'Nav Menu', 'innovecouture' ) ],
			'container'                                  => [ 'label' => __( 'Container', 'innovecouture' ) ],
			'toggle'                                     => [ 'label' => __( 'Toggle', 'innovecouture' ) ],
			'shortcode'                                  => [ 'label' => __( 'Shortcode', 'innovecouture' ) ],
			'testimonial-carousel'                       => [ 'label' => __( 'Testimonial Carousel', 'innovecouture' ) ],
			'woocommerce-menu-cart'                      => [ 'label' => __( 'Menu Cart', 'innovecouture' ) ],
			'woocommerce-cart'                			 => [ 'label' => __( 'Cart', 'innovecouture' ) ],
			'woocommerce-product-images'                 => [ 'label' => __( 'Product Images', 'innovecouture' ) ],
			'woocommerce-product-price'                  => [ 'label' => __( 'Product Price', 'innovecouture' ) ],
			'woocommerce-product-add-to-cart'            => [ 'label' => __( 'Add To Cart', 'innovecouture' ) ],
			'woocommerce-product-data-tabs'              => [ 'label' => __( 'Product Data Tabs', 'innovecouture' ) ],
			'woocommerce-product-rating'                 => [ 'label' => __( 'Product Rating', 'innovecouture' ) ],
			'woocommerce-product-additional-information' => [ 'label' => __( 'Product Additional Information', 'innovecouture' ) ],
			'paypal-button'                              => [ 'label' => __( 'Paypal Button', 'innovecouture' ) ],
			'theme-post-featured-image'                  => [ 'label' => __( 'Post Featured Image', 'innovecouture' ) ],
			'popup' /* Not a widget but same logic */    => [ 'label' => __( 'Popup', 'innovecouture' ) ],
			'post-comments'                              => [ 'label' => __( 'Post Comments', 'innovecouture' ) ],
			'wc-elements'                                => [ 'label' => __( 'WooCommerce Pages', 'innovecouture' ) ],
			'woocommerce-cart'                           => [ 'label' => __( 'WooCommerce Cart', 'innovecouture' ) ],
			'woocommerce-checkout-page'                  => [ 'label' => __( 'WooCommerce Checkout', 'innovecouture' ) ],
			'woocommerce-purchase-summary'               => [ 'label' => __( 'WooCommerce Purchase Summary', 'innovecouture' ) ],
			'woocommerce-my-account'                     => [ 'label' => __( 'WooCommerce My Account', 'innovecouture' ) ],
			'fibosearch'            				     => [ 'label' => __( 'FiboSearch', 'innovecouture' ) ],
			'wp-widget-dgwt_wcas_ajax_search'            => [ 'label' => __( 'FiboSearch Bar', 'innovecouture' ) ],
			'wp-widget-berocket_aapf_group'              => [ 'label' => __( 'AAPF Filters Group', 'innovecouture' ) ],
		];
	...
	public static function get_wc_mods_list() {
		...
		$wc_mods_list = [
			...
			'wc_products_new_badge' => [
				'label'       => __( '"New" badge for products.', 'innovecouture' ),
				'description' => __( 'Adds an option to allow marking products as new. The option is added on the products edit page under the advanced tab.', 'innovecouture' ),
			],
			'wc_products_gallery_in_loop' => [
				'label'       => __( 'Products Gallery in Loop.', 'innovecouture' ),
				'description' => __( 'Adds a gallery of product images to the products loop. The gallery (if exists) is shown when hovering over the product\'s thumbnail.', 'innovecouture' ),
			],
			'wc_custom_products_ordering' => [
				'label'       => __( 'Custom Products Ordering.', 'innovecouture' ),
				'description' => __( 'Custom sorting for product archives.', 'innovecouture' ),
			],
			// Only page-related mods need to use hyphens in the name, for other WC mods we can use underscores, as of PHP convention.
			'shipping_progress_bar_theme_mod'  => [
				'label' => __( 'Shipping Progress Bar', 'innovecouture' ),
				'description' => __( 'Toggles the "Free Shipping Progress Bar" funcionality.', 'innovecouture' ),
			],
		];
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'innovecouture' ) );
		}
	...
	*/
	...
	// Checks if the current document is a loop-item (part of a loop in Elementor's LoopBuilder).
	public static function doc_is_loop_item() {
		if ( ! self::is_elementor_pro_active() ) {
			return false;
		}

		$current_doc = \ElementorPro\Plugin::elementor()->documents->get_current();

		if ( ! $current_doc ) {
			return false;
		}

		return 'loop-item' === $current_doc->get_type();
	}

	public static function get_theme_custom_color_and_font_ids() {
		$ids = [];

		// Theme Custom Colors.
		$theme_custom_colors = self::options_exist( 'theme-custom-colors' );
		if ( is_array( $theme_custom_colors ) && ! empty( $theme_custom_colors ) ) {
			$ids[ 'theme_custom_colors' ] = array_column( $theme_custom_colors, '_id' );
		}

		// Theme Custom Fonts.
		$theme_custom_fonts = self::options_exist( 'theme-custom-fonts' );
		if ( is_array( $theme_custom_fonts ) && ! empty( $theme_custom_fonts ) ) {
			$ids[ 'theme_custom_fonts' ] = array_column( $theme_custom_fonts, '_id' );
		}
